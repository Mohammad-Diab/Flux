...
apps are now ported, the WPF pair is removed from this branch, and the `.WinUI` suffix is dropped
from the project names, so `Flux.sln` holds FluxCore, the tests, `Flux.Ui`, `FluxCast` and
`FluxRead` — all three WinUI. Where these notes name a WPF type, it lives in `master`, not here.
They cover all three projects; they stay in this folder because that is where they were written.
...
& "C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\amd64\MSBuild.exe" `
    FluxRead\FluxRead.csproj /t:Restore,Build /p:Configuration=Debug /p:Platform=x64
```
...
- `ByteFormat`, `TimeFormat` and `SettingsService` live in `Flux.Ui` and write the same settings.json
  the WPF app did, so the reference build in `dist/` shares it; `ShellViewModel.SessionRoot` became
  `Services/ReceptionPaths`, pointing at the same folder so a reception can move between the two.
