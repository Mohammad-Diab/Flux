...
public partial class LiveCaptureViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private ImageSource? _lastThumbnail;

    [ObservableProperty]
    private ImageSource? _regionPreview;

    [ObservableProperty]
    private ImageSource? _calibrationPreview;
    ...
    // WinUI decodes image sources off a stream, so the thumbnail lands a frame after the status it came with.
    private async Task ShowThumbnailAsync(byte[] png)
    {
        try
        {
            LastThumbnail = await BitmapConverter.FromPngAsync(png);
        }
        catch
        {
